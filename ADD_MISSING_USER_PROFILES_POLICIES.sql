...
-- 3. Add the basic policies that are missing
...
CREATE POLICY IF NOT EXISTS "users_can_insert_own_profile" ON user_profiles
    FOR INSERT
    WITH CHECK (id = auth.uid());

-- Allow users to see other profiles in their tenant
-- This uses a self-join which should not cause recursion
CREATE POLICY IF NOT EXISTS "users_can_view_tenant_profiles" ON user_profiles
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 
