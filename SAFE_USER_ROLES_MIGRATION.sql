-- SAFE USER ROLES MIGRATION
-- This migration preserves existing policies while adding new role functionality

-- 1. Add new role fields to user_profiles table WITHOUT dropping existing role column
...
-- 2. Update existing users to have proper access levels and platform flags
...
-- 5. Add a new role_name column that maps to the new role system
ALTER TABLE user_profiles 
...
-- 6. Create function to get user permissions (updated to work with both role systems)
...
-- 7. Add enhanced RLS policies that work with both old and new role systems
-- These will work alongside existing policies

-- Platform users can access any tenant data (NEW)
CREATE POLICY IF NOT EXISTS "platform_users_global_access" ON jobs
...
-- Enhanced technician policy that respects new role system
CREATE POLICY IF NOT EXISTS "enhanced_technician_access" ON jobs
...
CREATE POLICY IF NOT EXISTS "platform_users_global_access_leads" ON leads
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
...
CREATE POLICY IF NOT EXISTS "platform_users_global_access_accounts" ON accounts
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
...
CREATE POLICY IF NOT EXISTS "platform_users_global_access_contacts" ON contacts
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
...
-- 10. Set up a platform admin user - UPDATE THIS EMAIL TO YOUR ACTUAL ADMIN EMAIL
...
-- 11. Create a view for easier role management
...
-- Show a sample of contractor users
SELECT 
  tenant_id,
  COALESCE(role_name, role) as effective_role,
  COUNT(*) as user_count
...
GROUP BY tenant_id, COALESCE(role_name, role)
ORDER BY tenant_id, effective_role
LIMIT 10;
...
BEGIN
  ...
  RAISE NOTICE '1. Update the admin email in step 10 to set your platform admin user';
  RAISE NOTICE '2. Existing policies remain intact and functional';
  RAISE NOTICE '3. New role system is ready to use alongside legacy system';
  RAISE NOTICE '4. Run the verification queries to confirm setup';
END $$;