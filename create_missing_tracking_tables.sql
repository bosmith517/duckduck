-- Create missing tracking tables and fix existing ones
...
-- 8. Create basic RLS policies for job_status_updates
CREATE POLICY IF NOT EXISTS "Technicians can insert status updates" ON job_status_updates
    FOR INSERT 
    WITH CHECK (auth.uid() = technician_id);

CREATE POLICY IF NOT EXISTS "Technicians can view their own status updates" ON job_status_updates
    FOR SELECT 
    USING (auth.uid() = technician_id);

CREATE POLICY IF NOT EXISTS "Admins can view all status updates" ON job_status_updates
    FOR SELECT 
    USING (
        EXISTS (
...
-- 9. Create basic RLS policies for location_logs
CREATE POLICY IF NOT EXISTS "Technicians can view their own location history" ON location_logs
    FOR SELECT 
    USING (auth.uid() = technician_id);

CREATE POLICY IF NOT EXISTS "Admins can view all location logs" ON location_logs
    FOR SELECT 
    USING (
        EXISTS (
            SELECT 1 FROM user_profiles 
